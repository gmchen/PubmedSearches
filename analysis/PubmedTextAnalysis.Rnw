...
opts_chunk$set(fig.path='Figures/', fig.align='center', fig.show='hold', echo=FALSE, warning=FALSE, message=FALSE, results="hide", cache=FALSE, dev=c('pdf','postscript'))
@
...
<<generate_randomized_file_for_blinded_review, eval=FALSE>>=
journal.dfs <- lapply(names(all.texts), function(journalName) {
  texts <- all.texts[[journalName]]
  # Five-year intervals
  
  is.clinical.trial <- sapply(texts$publication.types, apply.clinical.trial.filters  )
  in.year.range <- texts$year %in% c(1976:1980, 2011:2015)
    
  current.df <- data.frame(
                title=texts$title[is.clinical.trial & in.year.range],
                year=texts$year[is.clinical.trial & in.year.range],
                journal=journalName
                )
  return(current.df)
})

names(journal.dfs) <- names(all.texts)

all.journal.df <- do.call(rbind, journal.dfs)

set.seed(100)
# randomize
all.journal.df <- all.journal.df[sample(1:nrow(all.journal.df)),]


write.table(
  all.journal.df,
  file = paste0("Randomized-Nov12-alljournals.csv"),
  col.names=FALSE,
  row.names=FALSE,
  sep=",")
@
...
Two independent raters, blinded to journal name and publication year assessed the use of patient-centric language in the titles of manuscripts reporting clinical trials published in JAMA and four other journals.

<<manual_annotation_generate_patient-centric_keywords_by_journal>>=
consensus.review.df <- read.csv("~/Google Drive/Doctoring 1A Capstone/Nov15-meeting/Post_Consensus_manual_review_Nov15.csv", fileEncoding="latin1")

rownames(consensus.review.df) <- consensus.review.df$X
consensus.review.df$X <- NULL

consensus.review.df.for.barplot <- data.frame(proportion=numeric(), n.hits=numeric(), n.nonhits=numeric(), timeframe=factor(levels=c("early", "late")), title.length=numeric(), journal=character(), lcb.95=numeric(), ucb.95=numeric())

for(journalName in levels(consensus.review.df$journal)) {
  mean.consensus.review.hits.early <- mean(
    consensus.review.df$consensus.category[
                  consensus.review.df$time.period == "early" &
                  consensus.review.df$journal == journalName
                  ] == "Patient-centric"
    )
  num.consensus.review.hits.early <- sum(
    consensus.review.df$consensus.category[
                  consensus.review.df$time.period == "early" &
                  consensus.review.df$journal == journalName
                  ] == "Patient-centric"
    )
  num.consensus.review.nonhits.early <- sum(
    consensus.review.df$consensus.category[
                  consensus.review.df$time.period == "early" &
                  consensus.review.df$journal == journalName
                  ] == "Non-Patient-Centric"
  ...
  mean.consensus.review.hits.late <- mean(
    consensus.review.df$consensus.category[
                  consensus.review.df$time.period == "late" &
                  consensus.review.df$journal == journalName
                  ] == "Patient-centric"
    )
  num.consensus.review.hits.late <- sum(
    consensus.review.df$consensus.category[
                  consensus.review.df$time.period == "late" &
                  consensus.review.df$journal == journalName
                  ] == "Patient-centric"
    )
  num.consensus.review.nonhits.late <- sum(
    consensus.review.df$consensus.category[
                  consensus.review.df$time.period == "late" &
                  consensus.review.df$journal == journalName
                  ] == "Non-Patient-Centric"
  ...
  mean.title.length.early <- mean(
    consensus.review.df$title.length[
                  consensus.review.df$time.period == "early" &
                  consensus.review.df$journal == journalName
                  ]
    )
  ...
  df.to.add <- data.frame(
    proportion=c(mean.consensus.review.hits.early, mean.consensus.review.hits.late),
    n.hits=c(num.consensus.review.hits.early, num.consensus.review.hits.late),
    n.nonhits=c(num.consensus.review.nonhits.early, num.consensus.review.nonhits.late),
    timeframe=c("early", "late"),
    title.length=c(mean.title.length.early, mean.title.length.late),
    journal=journalName)
  consensus.review.df.for.barplot <- rbind(consensus.review.df.for.barplot, df.to.add)
  
}

consensus.review.df.for.barplot$journal <- factor(
                                            consensus.review.df.for.barplot$journal,
                                            levels=c("jama", "lancet", "annals", "bmj", "nejm"))

consensus.review.df.for.barplot$lcb.95 <- NA
consensus.review.df.for.barplot$ucb.95 <- NA

sapply(1:nrow(consensus.review.df.for.barplot), function(rowi) {
  prop.test.out <- prop.test(consensus.review.df.for.barplot$n.hits[rowi], consensus.review.df.for.barplot$n.hits[rowi] + consensus.review.df.for.barplot$n.nonhits[rowi])
  consensus.review.df.for.barplot$lcb.95[rowi] <<- prop.test.out$conf.int[1]
  consensus.review.df.for.barplot$ucb.95[rowi] <<- prop.test.out$conf.int[2]
})

# Stats for proportions
chisq.test(as.matrix(consensus.review.df.for.barplot[consensus.review.df.for.barplot$journal == "jama",c("n.hits", "n.nonhits")]))

chisq.test(as.matrix(consensus.review.df.for.barplot[consensus.review.df.for.barplot$journal == "lancet",c("n.hits", "n.nonhits")]))

chisq.test(as.matrix(consensus.review.df.for.barplot[consensus.review.df.for.barplot$journal == "annals",c("n.hits", "n.nonhits")]))

chisq.test(as.matrix(consensus.review.df.for.barplot[consensus.review.df.for.barplot$journal == "bmj",c("n.hits", "n.nonhits")]))

chisq.test(as.matrix(consensus.review.df.for.barplot[consensus.review.df.for.barplot$journal == "nejm",c("n.hits", "n.nonhits")]))


# Stats for title length
t.test(title.length ~ time.period, consensus.review.df[consensus.review.df$journal=="jama",], alternative="less")
t.test(title.length ~ time.period, consensus.review.df[consensus.review.df$journal=="lancet",], alternative="less")
t.test(title.length ~ time.period, consensus.review.df[consensus.review.df$journal=="annals",], alternative="less")
t.test(title.length ~ time.period, consensus.review.df[consensus.review.df$journal=="bmj",], alternative="less")
t.test(title.length ~ time.period, consensus.review.df[consensus.review.df$journal=="nejm",], alternative="less")
@

Number of articles of all types from all journals: \Sexpr{sum(sapply(all.texts, function(x) length(x$title)))}
Number of articles of all types from each journals: \Sexpr{sapply(all.texts, function(x) length(x$title))}

Number of JAMA articles of all types: \Sexpr{length(all.texts$jama$title)}

Number of clinical trial titles by journal:
\Sexpr{table(consensus.review.df$journal)}

Average increase in character count:
\Sexpr{consensus.review.df.for.barplot$title.length[c(2,4,6,8,10)] - consensus.review.df.for.barplot$title.length[c(1,3,5,7,9)]}

Absolute change in proportion of patient-centric titles:
\Sexpr{consensus.review.df.for.barplot$proportion[c(2,4,6,8,10)] - consensus.review.df.for.barplot$proportion[c(1,3,5,7,9)]}

<<consensus_review_patient-centric_keywords_by_journal, fig.width=5, fig.height=5, out.width="0.6\\textwidth">>=

sig.label.df <- data.frame(journal = c("jama", "lancet", "annals", "bmj"),
                       proportion = c(0.60, 0.54, 0.55, 0.58))
nonsig.label.df <- data.frame(journal = c("nejm"),
                       proportion = c(0.32))

ggplot(data=consensus.review.df.for.barplot, aes(x=journal, y=proportion)) +
  geom_errorbar(aes(group=timeframe, ymin=lcb.95, ymax=ucb.95), width=0.5, position=position_dodge(0.9), col="darkgray") +
  geom_bar(stat='identity', position=position_dodge(),  aes(fill=timeframe)) + 
  ...
  ylab("Percent of titles annotated as patient-centric") +
  xlab("") +
  geom_text(data = sig.label.df, label = "*", size=7)
  #geom_text(data = nonsig.label.df, label = "n.s.", size=3)
@
...
#Remove duplicates based on title, journal, and year
clinical.trials.merged.df <- clinical.trials.merged.df[!duplicated(clinical.trials.merged.df[,c(1,2,3)]),]

patient.keyword.df.for.barplot <- data.frame(proportion=numeric(), n.hits=numeric(), n.nonhits=numeric(), timeframe=factor(levels=c("early", "late")), title.length=numeric(), journal=character(), lcb.95=numeric(), ucb.95=numeric())
...
We applied a keyword search for the percent of titles containing at least one of the following terms: patient, patients, adult, adults, woman, women, man, men, child, children, infant, infants, male, males, female, females, donor, donors
...
Monograms such as ``randomized'', ``trial'', ``outcomes'', and ``risk'' showed significant increases with the following mean annual change in frequency per 10,000 words:
\Sexpr{frequencyAnalysis$jama$title$all$monogram$sig.lm.coefficients[c("randomized", "trial", "outcomes", "risk")]}

Bigrams such as ``a patient'' and ``patient with'' decreased, and ``in patients'' and ``patients with'' increased
\Sexpr{frequencyAnalysis$jama$title$all$bigram$sig.lm.coefficients[c("a patient", "patient with", "in patients", "patients with")]}

Bigrams such as ``the elderly'' decreased, and ``older patients'' and ``older adults'' increased
\Sexpr{frequencyAnalysis$jama$title$all$bigram$sig.lm.coefficients[c("the elderly", "older patients", "older adults")]}

Bigrams such as ``caused by'' and ``cause of'' decreased, and ``association between'' and ``association of'' increased
\Sexpr{frequencyAnalysis$jama$title$all$bigram$sig.lm.coefficients[c("caused by", "cause of", "association between", "association of")]}

The monogram ``diabetes'' significantly increased, and ``diabetic'' significantly decreased:
\Sexpr{frequencyAnalysis$jama$title$all$monogram$sig.lm.coefficients[c("diabetes", "diabetic")]}
...
<<barplots_epidemiological_terms, fig.width=8, fig.height=8, eval=TRUE>>=
...
<<barplots_patient_vs_patients, fig.width=8, fig.height=8, eval=TRUE>>=
grid.arrange(
...
<<barplots_elderly, fig.width=8, fig.height=8, eval=TRUE>>=
grid.arrange(
...
<<barplots_diabetes, fig.width=8, fig.height=4, eval=TRUE>>=
grid.arrange(
...
<<barplots_causation, fig.width=8, fig.height=4, eval=TRUE>>=
grid.arrange(
...
<<barplots_kidney, fig.width=8, fig.height=4, eval=TRUE>>=
grid.arrange(
...
<<barplots_mental_health, fig.width=8, fig.height=4, eval=TRUE>>=
grid.arrange(
...
<<jama-plots-together, fig.width=10, fig.height=6>>=
p1 <- get.top.terms.barplot(frequencyAnalysis$jama$title$all$monogram)
p1 <- p1 + scale_y_continuous(limits=c(-20,40), breaks=c(-20,-10,0,10,20,30,40))
p2 <- get.top.terms.barplot(frequencyAnalysis$jama$title$all$bigram)
p2 <- p2 + scale_y_continuous(limits=c(-15,20), breaks=c(-15,-10,-5,0,5,10,15,20))
grid.arrange(p1, p2, ncol=2, widths=c(1, 1.085))
#ggsave("jama_plots_together.pdf", width=6.5, height=5.7, arrangeGrob(p1, p2, layout_matrix = matrix(c(1,2), ncol=2)))
@

<<jama-figure-1A, fig.width=5, fig.height=6>>=
p1
@

<<jama-figure-1B, fig.width=5, fig.height=6>>=
p2
@

<<mega-plot-jama, fig.width=12, fig.height=14>>=
grid.arrange(
  get.barplot(frequencyAnalysis$jama$title$all$monogram$term.freq.per.100000, "randomized"),
