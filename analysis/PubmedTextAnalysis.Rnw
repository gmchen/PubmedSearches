...
for(journalName in names(all.texts)) {
  ...
  year.counts.with.clinical.trial <- sapply(1976:2015, function(year) {
    mean(sapply(texts$publication.types, apply.clinical.trial.filters))
    })
...
ggplot(data=patient.keyword.df.for.barplot, aes(x=journal, y=proportion, fill=timeframe)) +
  ...
  scale_fill_manual(name="Time Period", values=c('#999999','#E69F00')) + 
  theme_classic() + 
  theme(plot.title = element_text(hjust=0.5, face="bold"), axis.text.y = element_text(size=15)) + 
  scale_x_discrete(labels=c("JAMA", "Lancet", "Ann Intern Med.", "BMJ", "N Engl J Med.")) +
  ylab("Proportion") + 
  xlab("Journal Name")
@

<<get_corpus_texts_and_frequency_analysis, cache=TRUE>>=
if(file.exists("corpusTexts.RData") && file.exists("frequencyAnalysis.RData")) {
  load("corpusTexts.RData")
  load("frequencyAnalysis.RData")
  } else {
  corpusTexts <- list(jama=list())
  
  for(journalName in c("jama")) {
    if(journalName == "jama") {
      texts <- jamaTexts
    } else if (journalName == "annals") {
      texts <- annalsTexts
...
For this analysis, we excluded the following words: "piece", "mind", "jama", "patient", "page", "cover", "council". A To-Do item is to compile an exhuastive list of these headers and remove them at the beginning of the analysis.
...
<<barplots_patients_with, fig.width=8, fig.height=4>>=
grid.arrange(
...
<<barplots_a_in_patients, fig.width=8, fig.height=4>>=
grid.arrange(
