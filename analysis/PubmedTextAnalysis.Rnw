...
patient.keyword.df.for.barplot <- data.frame(proportion=numeric(), timeframe=factor(levels=c("1976-1980", "2011-2015")), title.length=numeric(), journal=character())

for(journalName in names(all.texts)) {
  num.patient.keyword.hits.early <- mean(
    clinical.trials.merged.df$patient.keyword.hit[
                  clinical.trials.merged.df$year %in% 1976:1980 &
                  clinical.trials.merged.df$journal == journalName
                  ] == TRUE
    )
  num.patient.keyword.hits.late <- mean(
    clinical.trials.merged.df$patient.keyword.hit[
                  clinical.trials.merged.df$year %in% 2011:2015 &
                  clinical.trials.merged.df$journal == journalName
                  ] == TRUE
    )
  ...
  df.to.add <- data.frame(
    proportion=c(num.patient.keyword.hits.early, num.patient.keyword.hits.late),
    timeframe=c("1976-1980", "2011-2015"),
...
ggplot(data=patient.keyword.df.for.barplot, aes(x=journal, y=proportion, fill=timeframe)) +
  ...
  scale_fill_manual(name="Time Period", values=c('gray','#E69F00')) + 
  theme_classic() + 
    ...
    lapply(corpusTextByJournal, function(corpusTextByTextType) {
        lapply(corpusTextByTextType, function(corpusTextByPubType) {
          out.vals <- lapply(c("monogram", "bigram"), function(gramtype) {
            ...
            # only include terms that have occurred at least 1/1000000 (0.01%) of the time
...
On some years, JAMA has implemented some headers in their titles whose words we sought to exclude. Some examples are the following:
...
<<>>=
get.barplot <- function(tdm.freq.per.100000, termInTitle, direction=c("positive", "negative"), ylab="Frequency per 100000 words") {
...
<<barplots_epidemiological_terms, fig.width=8, fig.height=8>>=
...
<<barplots_patient_vs_patients, fig.width=8, fig.height=8>>=
grid.arrange(
...
<<barplots_elderly, fig.width=8, fig.height=8>>=
grid.arrange(
...
<<barplots_diabetes, fig.width=8, fig.height=4>>=
grid.arrange(
...
<<barplots_causation, fig.width=8, fig.height=4>>=
grid.arrange(
...
<<barplots_kidney, fig.width=8, fig.height=4>>=
grid.arrange(
...
<<barplots_mental_health, fig.width=8, fig.height=4>>=
grid.arrange(
...
get.top.terms.barplot <- function(stat.list, increase.n=25, decrease.n=25) {
  ...
  ggplot(top.words.df, aes(x=term, y=coef, fill=term)) +
      ...
      coord_flip() +
      xlab("") + 
      ylab("Annual change in frequency per 100000 terms") +
...
grid.arrange(
  get.barplot(frequencyAnalysis$bmj$title$all$monogram$term.freq.per.100000, "randomized"),
